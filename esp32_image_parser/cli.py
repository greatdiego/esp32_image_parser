...
from makeelf.elf import (
    ELF, EM, ELFDATA, SHF, SHT, PF, PT, STB, STT, Elf32_Phdr
)
from esptool.bin_image import LoadFirmwareImage
from .esp32_firmware_reader import (
    read_partition_table, print_verbose, dump_bytes
)
...
def image2elf(filename, output_file, verbose=False):
    ...
    for name in section_data.keys():
        ...
        if name in sect_attr_map:
            ...
            elf._append_section(
                name, data, addr, SHT.SHT_PROGBITS, flg,
                sect['Lk'], sect['Inf'], sect['Al'], sect['ES']
            )
        else:
...
def add_elf_symbols(elf):
    ...
    for line in lines:
        ...
        # ABS
        elf.append_symbol(
            sym_name, 0xfff1, sym_val, sym_size,
...
def main():
    ...
    arg_parser.add_argument(
        'action',
        choices=['show_partitions', 'dump_partition', 'create_elf', 'dump_nvs'],
        help='Action to take'
    )
